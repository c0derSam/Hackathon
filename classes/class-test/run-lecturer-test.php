...
** File name : classes/class-test/run-lecturer-test.php

** About : this module displays the test questions to be answered in lecturer mode

*/ 

/**PSUEDO ALGORITHM
 * *
 * define the run lecturer test class
...
class runLecturerTest{
       ...
       $(document).ready(function() {
            ...
            $(".timer").startTimer({
                onComplete:function(element){
                        ...
                        window.location.href = "lecturer-test-initializer.php?classEncryptedId='.$this->class_encrypted_id.'&&testEncryptedId='.$this->test_encrypted_id.'";
    ...
    public function beginForm(){
        ...
        <form action = "process-lecturer-test.php" method="post">
...
$run_lecturer_test = new runLecturerTest();
...
if(isset($_SESSION['lecturer_session'])){
    ...
    $_GET['classEncryptedId'] !== "" && isset($_GET['mins']) && $_GET['mins'] !== "" && $_GET['secs'] !== "" && isset($_GET['total']) && $_GET['total'] !== ""){
        ...
        $run_lecturer_test->fetchTestData();

        $run_lecturer_test->header();

        if($run_lecturer_test->status == "active"){


            $run_lecturer_test->testSubheading();

            $run_lecturer_test->cacheTestSubheading();

            $run_lecturer_test->beginForm();

            if($run_lecturer_test->total == 10){

                $run_lecturer_test->questionsDbConn();

                $run_lecturer_test->fetchQuestionOne();

                $run_lecturer_test->fetchQuestionTwo();

                $run_lecturer_test->fetchQuestionThree();

                $run_lecturer_test->fetchQuestionFour();

                $run_lecturer_test->fetchQuestionFive();

                $run_lecturer_test->fetchQuestionSix();

                $run_lecturer_test->fetchQuestionSeven();

                $run_lecturer_test->fetchQuestionEight();

                $run_lecturer_test->fetchQuestionNine();

                $run_lecturer_test->fetchQuestionTen();

            }elseif($run_lecturer_test->total == 20){
                ...
                $run_lecturer_test->fetchQuestionEleven();

                $run_lecturer_test->fetchQuestionTwelve();

                $run_lecturer_test->fetchQuestionThirteen();

                $run_lecturer_test->fetchQuestionFourteen();

                $run_lecturer_test->fetchQuestionFifteen();

                $run_lecturer_test->fetchQuestionSixteen();

                $run_lecturer_test->fetchQuestionSeventeen();

                $run_lecturer_test->fetchQuestionEighteen();

                $run_lecturer_test->fetchQuestionNineteen();

                $run_lecturer_test->fetchQuestionTwenty();

            }

            $run_lecturer_test->endForm();

            //session_regenerate_id(true);

            $_SESSION['test-encrypted-id'] = $run_lecturer_test->test_encrypted_id;

            $_SESSION['class-encrypted-id'] = $run_lecturer_test->class_encrypted_id;

        }else{

            $run_lecturer_test->displayTestNotActive();
