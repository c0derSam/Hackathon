...
** File name : hod-admin/fetch-100level-students.php

** About : this module displays fetches all the students in 100 level from the database

*/

/**PSUEDO ALGORITHM
 * *
 * initiate the fetch 100 level students class
 * fetch the all the 100 level students query
 * 
 * *
 */
...
//initiate the fetch 100 level students class
class fetchFirstLevelStudents{
    
    public $status;

    public $first_level_students_rows;

    //fetch the all the 100 level students query
    public function fetchFirstLevelStudentsQuery(){

        include('../resources/database/users-db-connection.php');//conn1 

       $fetch_students_data_query = "SELECT * FROM students WHERE level = '100'";
       ...
       CacheManager::setDefaultConfig(new ConfigurationOption([
        ...
        while($row = $cached_student_result->fetch_assoc()){
            ...
            $this->first_level_students_rows[] = $row;
...
$fetch_first_level_student = new fetchFirstLevelStudents();

$fetch_first_level_student->fetchFirstLevelStudentsQuery();
